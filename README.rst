...
- Put note on top of each theme page that link back to themata ` <>`_
- Add `edit on 'website'` to webpages e.g. edit on github ` <>`_
- Add raw_source_root option for the edit on link and edit_on_text for the text to display to theme option` <>`_